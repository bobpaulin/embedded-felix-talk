...
public class SimConfig implements GameConfig {	
	...
	private SoarConfigFactoryService soarConfigFactory;
	...
	private SimConfig(Config config) {
		this.config = config;
		this.soarConfigFactory = OsgiMain.getInstance().getService(SoarConfigFactoryService.class);
		...
		switch (game) {
		case TANKSOAR:
			gameConfig = soarConfigFactory.getConfig("tanksoar").getGameConfig();
			break;
		case EATERS:
